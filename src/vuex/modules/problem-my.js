...
const actions = {
  addMyProblem({ commit, state, rootState }, queryParams){
    return new Promise((resolve, reject) => {
      api.requestProblem({
        company_id: rootState.user_info.company_id,
        count: rootState.request_count,
        start: state.start,
        changer_id: rootState.openid,
        ...queryParams
        },
        problems => {
          if(problems.length < rootState.request_count) commit(types.UPDATE_MY_PROBLEM_LOAD_END, true)

          commit(types.UPDATE_MY_PROBLEM_START, state.start + problems.length)
